...
#Combine datasets with all IS mean by genus for all sites - one df for each shared genus

#Try with acanthurus
acanth <- data_vir_sum_genus %>% dplyr::mutate(site = "vir") %>% filter(genus == "Acanthurus") %>%
    rbind(data_nca_sum_genus %>% dplyr::mutate(site = "nca") %>% filter(genus == "Acanthurus")) %>%
    rbind(data_mari_sum_genus %>% dplyr::mutate(site = "mari") %>% filter(genus == "Acanthurus")) %>%
    rbind(data_haw_sum_genus %>% dplyr::mutate(site = "haw") %>% filter(genus == "Acanthurus")) %>%
    rbind(data_mad_sum_genus %>% dplyr::mutate(site = "mad") %>% filter(genus == "Acanthurus"))
#But problem here is not standardized

#So try from the matrices
...
colfunc4 <- colorRampPalette(c("darkviolet","royalblue1","turquoise1","chartreuse3", "gold","orange","brown2","lightpink2")) #,"slateblue4", 
plot(rep(1,24),col=(colfunc4(24)),pch=19,cex=4) #plot to see the colours
...
ggplot(data = subset(acanth, Freq > 0.02), aes(x=site, y=Freq, fill = Var1))+
...
ggplot(data = subset(synodus, Freq > 0.02), aes(x=site, y=Freq, fill = Var1))+
  geom_bar(stat="identity",  color="white")+
...
ggplot(data = subset(neoniphon, Freq > 0.02), aes(x=site, y=Freq, fill = Var1))+
...
ggplot(data = subset(sargo, Freq > 0.02), aes(x=site, y=Freq, fill = Var1))+
  geom_bar(stat="identity", colour = "white") +
  #scale_fill_manual(values = manual_scale_colors) +
  scale_fill_discrete()+
  labs(y = "Interaction strength", x = "") + 
  ggtitle("d) Sargocentron") +
  theme_minimal()

ggplot(data = subset(cephalopholis, Freq > 0.02), aes(x=site, y=Freq, fill = Var1))+
  geom_bar(stat="identity", colour = "white") +
  scale_fill_manual(values = manual_scale_colors) +
  labs(y = "Interaction strength", x = "") + 
...
ggplot(data = subset(genus_com, Freq > 0.02), aes(x="", y=Freq, fill = Var1))+
  geom_bar(stat="identity", colour = "white") +
